...
export function ReferenceWedding({ invitation, design, locale, onChange, selected, onSelect, onPartsChange }: Props) {
  ...
  const map = () => <WeddingPart id="map-button" label="Кнопка карты" kind="widget"><a className={css.button} href={safeWeddingLink(invitationMapUrl(invitation))} target="_blank" rel="noopener noreferrer" onClick={e => { if (onChange) e.preventDefault(); }}><Text id="map-label" text={english ? "GET DIRECTIONS" : kazakh ? "КАРТАНЫ АШУ" : "ПОСМОТРЕТЬ НА КАРТЕ"} /></a></WeddingPart>;

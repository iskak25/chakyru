...
function Calendar({ invitation, english = false }: { invitation: Invitation; english?: boolean }) {
  ...
  return <WeddingPart id="calendar" label="Календарь — дата свадьбы" kind="date" className={css.calendar}>
    <div className={css.month}>{parsed.toLocaleDateString(english ? "en-US" : "ru-RU", { month: "long" })}{english ? ` ${y}` : ""}</div>
    <div className={css.calendarGrid}>{(english ? ["M", "T", "W", "T", "F", "S", "S"] : ["ПН", "ВТ", "СР", "ЧТ", "ПТ", "СБ", "ВС"]).map((d, i) => <small key={`day-${i}`}>{d}</small>)}
...
function Countdown({ invitation, english }: { invitation: Invitation; english: boolean }) {
  ...
  return <WeddingPart id="countdown" label="Обратный отсчёт" kind="date" className={css.countdown}>
    {values.map((n, i) => <div key={i}><strong>{now === null ? "—" : String(n).padStart(2, "0")}</strong><small>{(english ? ["DAYS", "HOURS", "MINUTES", "SECONDS"] : ["ДНЕЙ", "ЧАСОВ", "МИНУТ", "СЕКУНД"])[i]}</small></div>)}
  </WeddingPart>;
}
  ...
  const rsvp = () => section("rsvp", <>{heading("rsvp", english ? "Will you be there?" : kazakh ? "Тойға келесіз бе?" : d === "tuscany" ? "Подтверждение присутствия" : "Анкета гостя")}{d === "rose" ? <Flourish /> : null}<Text id="rsvp-description" text={english ? "Kindly respond so we can finalize our preparations for this magical evening." : kazakh ? "Аты-жөніңізді жазыңыз" : "Подтвердите, пожалуйста,\nваше присутствие"} className={css.body} />
    <form id={formId} className={css.form} onSubmit={async e => {
      ...
      try {
        ...
        if (!response.ok) throw new Error(english ? "Could not send your response. Please try again." : "Не удалось отправить ответ. Попробуйте ещё раз.");
        setStatus("sent");
      } catch (err) { setStatus("error"); setError(err instanceof Error ? err.message : "Ошибка отправки"); }
    }}>
      <WeddingPart id="rsvp-attendance" label="Варианты присутствия" kind="widget"><fieldset><legend className={css.srOnly}>{english ? "Attendance" : "Присутствие"}</legend>{(["yes", "no", "maybe"] as const).slice(0, d === "sage" ? 3 : 2).map((value, i) => <label className={css.radio} key={value}><input type="radio" name="attendance" value={value} defaultChecked={i === 0} /><Text id={`rsvp-option-${value}`} text={(english ? ["Joyfully accept", "Regretfully decline", "Maybe"] : kazakh ? ["Иә, келемін", "Жоқ, келе алмаймын", "Жұбыммен келемін"] : ["С радостью приду", "К сожалению, не смогу", "Пока не знаю"])[i]} /></label>)}</fieldset></WeddingPart>
      <label><Text id="rsvp-name-label" text={english ? "Your name" : "Ваше имя и фамилия"} className={css.formLabel} /><WeddingPart id="rsvp-name" label="Поле имени" kind="widget" fallback="Введите имя"><input name="name" aria-label={english ? "Your name" : "Ваше имя и фамилия"} autoComplete="name" required maxLength={120} placeholder={weddingStyle(invitation, "rsvp-name", "placeholder") || (english ? "Your name" : "Введите имя")} /></WeddingPart></label>
      <label><Text id="rsvp-guests-label" text={english ? "Number of guests" : "Количество гостей"} className={css.formLabel} /><WeddingPart id="rsvp-guests" label="Количество гостей" kind="widget"><input name="guests" aria-label="Количество гостей" type="number" min="1" max="20" defaultValue="1" required /></WeddingPart></label>
      ...
      {d === "rose" || d === "tuscany" ? <label><Text id="rsvp-note-label" text="Ваши пожелания" className={css.formLabel} /><WeddingPart id="rsvp-note" label="Поле пожеланий" kind="widget" fallback="Напишите несколько тёплых слов"><textarea name="note" aria-label="Ваши пожелания" maxLength={2000} placeholder={weddingStyle(invitation, "rsvp-note", "placeholder") || "Напишите несколько тёплых слов"} /></WeddingPart></label> : null}
      ...
      {status === "sent" ? <p role="status" className={css.feedback}>{invitation.id.startsWith("preview") || invitation.id === "demo" ? (english ? "This is a preview. Your response was not sent." : "Это предпросмотр. Ответ не отправлен.") : english ? "Thank you! Your response has been received." : "Спасибо! Ваш ответ отправлен."}</p> : null}{status === "error" ? <p role="alert" className={css.feedback}>{error}</p> : null}
    </form></>, css.rsvp);

  let content: ReactNode;
  if (d === "rose") content = <>
    {section("hero", <>{heading("hero", "Наша свадьба")}{photo("hero", css.wavePhoto)}{names()}{eventDate()}<Text id="hero-welcome" text="С нетерпением ждём\nвстречи с вами!" className={css.body} /><Text id="timer-title" text="ДО СВАДЬБЫ ОСТАЛОСЬ:" className={css.smallTitle} /><Countdown invitation={invitation} english={false} /></>, css.hero)}
    ...
    {section("hero", <>{eventDate()}{heading("hero", "Наша Свадьба", true)}{names()}{photo("hero", css.wavePhoto)}</>, css.hero)}{intro("Родные и близкие!", "6 сентября состоится долгожданное и радостное событие — наша свадьба!\n\nЭтот тёплый день мы будем счастливы провести в кругу самых близких и родных людей, в праздничной и душевной атмосфере.")}{photo("bottom")}{photo("couple")}<Calendar invitation={invitation} />{photo("flowers")}{venue()}{program()}{rsvp()}
  </>;

...
export function PinterestInvite({ invitation: inv, design, locale, onChange, selected, onSelect, onPartsChange }: Props) {
  ...
  const dotted=date?.toLocaleDateString("ru-RU") || inv.date;
  const formId=useId();
  ...
  const rsvp=() => section("rsvp",<>{title("rsvp",ru?"Анкета гостя":"Сауалнама",key!=="blue")}{text("rsvp-hint",ru?"Будем рады видеть вас на нашем празднике. Подтвердите, пожалуйста, присутствие.":"Жұбыңызбен келетін болсаңыз, есімдеріңізді бірге жазуыңызды өтінеміз!",css.body)}
    <form id={formId} className={css.form} onSubmit={async e=>{
      ...
      <label>{text("guest-name-label",ru?"Ваше имя и фамилия":"АТЫ-ЖӨНІҢІЗ",css.formLabel)}<WeddingPart id="guest-name" label="Поле имени" kind="widget" fallback="Имя"><input name="name" aria-label="Имя гостя" required maxLength={120} autoComplete="name" placeholder={weddingStyle(inv,"guest-name","placeholder") || (ru?"Ваше имя":"ЕСІМІҢІЗ")} /></WeddingPart></label>
      ...
      {formState==="sent"?<p role="status">{inv.id==="demo" || inv.id.startsWith("preview")?"Это предпросмотр. Ответ не отправлен.":"Спасибо! Ваш ответ отправлен."}</p>:null}{formState==="error"?<p role="alert">Не удалось отправить ответ. Попробуйте ещё раз.</p>:null}
    </form>
  </>,css.rsvp);
  ...
  if(photo){
    content=<div className={`${css.card} ${css[key]}`}>
      ...
      {key==="silkCard"?<>
        {text("save-date","SAVE THE DATE",css.cardOverline)}{ornament("top-divider")}{text("marrying","WIR HEIRATEN!",css.cardMarrying)}<Heart className={css.cardHeart} fill="currentColor"/>{text("message","Bitte haltet euch\ndiesen besonderen Tag\nfür uns frei.",css.cardMessage,"message")}{name()}{ornament("middle-divider")}<WeddingPart id="event-date" label="Дата свадьбы" kind="date" className={css.cardDate}><small>{date?.toLocaleDateString("de-DE",{weekday:"long"}).toUpperCase()}</small><span>{dotted.replaceAll(".","  |  ")}</span></WeddingPart>{ornament("bottom-divider")}{text("closing","Wir freuen uns auf euch!",css.cardClosing)}
      </>:key==="nikahCard"?<>
        ...
        {text("heading","WEDDING INVITATION",css.cardHeading)}{name()}<WeddingPart id="event-date" label="Дата свадьбы" kind="date" className={css.cardDate}><small>{date?.toLocaleDateString("en-US",{weekday:"long"})}</small><div><strong>{date?.toLocaleDateString("en-US",{month:"long"}).toUpperCase()}</strong><b>{date?.getDate()}</b><i>{date?.getFullYear()}</i></div><small>AT {inv.time}</small></WeddingPart>{text("venue",design.venue,css.cardVenue,"venue")}{inv.mapUrl?<WeddingPart id="map-button" label="Местоположение" kind="widget" className={css.cardMap}><a href={mapHref} target="_blank" rel="noopener noreferrer" onClick={e=>{if(onChange)e.preventDefault();}}><MapPin/>{text("map-label","LOCATION")}</a></WeddingPart>:null}
      </>:<>
        {text("save-word","SAVE",css.saveWord)}{text("the-word","the",css.theWord)}{text("date-word","DATE",css.dateWord)}{name()}{image("hero",css.newspaperPhoto)}{text("invited","YOU ARE INVITED",css.cardHeading)}{text("message","we would be happy to announce the day we are finally going to become husband and wife. we hope that our joy can be yours as well",css.cardMessage,"message")}<WeddingPart id="event-date" label="Дата свадьбы" kind="date" className={css.cardDate}>{date?.toLocaleDateString("en-US",{month:"short",day:"numeric"}).toUpperCase()} | {inv.time}</WeddingPart>{text("address",design.address,css.cardAddress,"address")}
      </>}
    </div>;

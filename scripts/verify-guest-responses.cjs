...
function load(file, requireMock, fetchMock) {
  ...
  new Function('require', 'module', 'exports', 'fetch', code)(requireMock, mod, mod.exports, fetchMock);
  return mod.exports;
}
...
(async () => {
  ...
  const { GuestWishForm } = load('components/GuestWishForm.tsx', name => name === 'react' ? { useState(initial) { const i = index++; if (!(i in hooks)) hooks[i] = initial; return [hooks[i], value => { hooks[i] = value; }]; } } : require(name), async (url, options) => { requests.push(JSON.parse(options.body)); return { ok: success, json: async () => ({ wish: { id: 'saved' } }) }; });
  ...
  hooks = ['Guest', 'Preview text', 'idle']; tree = render('preview-test');
  await nodes(tree).find(n => n.type === 'form').props.onSubmit({ preventDefault() {} });
